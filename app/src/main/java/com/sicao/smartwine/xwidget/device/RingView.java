...
public class RingView extends View {
     ...
     * 圆环 宽度
     */
    private final int mHeartPaintWidth = 15;
    ...
    final int start = 10086;
    final int stop = 10087;
    ...
    private void init() {
        ...
        if (!isInEditMode()) {
            // 造成错误的代码段
            mRingPaint.setColor(mContext.getResources().getColor(R.color.get_code_color));
        }
        mRingPaint.setStrokeWidth(mHeartPaintWidth);
        mRingPaint.setStyle(Style.STROKE);
        mRingAnimPaint = new Paint(mRingPaint);
        mRingAnimPaint.setColor(mContext.getResources().getColor(R.color.write));
        ...
        if (!isInEditMode()) {
            mHeartBeatPaint.setColor(mContext.getResources().getColor(R.color.write));
        }
